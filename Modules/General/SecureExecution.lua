...
local function wipeButtons()
	for key, property in pairs(mod) do
		if type(property) == "table" and property.GetObjectType and property:GetObjectType() == "Button" and key ~= "blockref" then
                ...
                args = {
                    ...
                    createButton = {
                        ...
                        func = function()
							...
							for action in pairs(actionTypes) do
								local attributes = getActionAttributes(action)
								buttonConfig.tabs[1].attributes[action] = {}

								for _, attr in ipairs(attributes) do
									buttonConfig.tabs[1].attributes[action][attr] = ""
								end
							end
				...
				args = {
					...
					addTab = {
						...
						func = function()
							...
							for action in pairs(actionTypes) do
								local attributes = getActionAttributes(action)
								tabConfig.attributes[action] = {}

								for _, attr in ipairs(attributes) do
									tabConfig.attributes[action][attr] = ""
								end
							end
	...
	for _, info in ipairs(db.buttons) do
		for _, tab in ipairs(info.tabs) do
			if not tab.attributes.macro then
				for action in pairs(actionTypes) do
					local attributes = getActionAttributes(action)
					tab.attributes[action] = {}

					for _, attr in ipairs(attributes) do
						tab.attributes[action][attr] = ""
					end
				end
			end
		end
...
function mod:SetButtonIcon(button, info, tabsIconInfo)
	local enabled = info.icon.enabled
...
function mod:GetFrame(button, scaledWidth, scaledHeight, screenWidth, screenHeight, startPos)
	if button.targetSecureFrame then return end
				...
				for _, plate in pairs(GetNamePlates()) do
					...
					if frame and unit and func(frame, unit, true) then
						if tabInfo.targetName then
							...
							local blockRef = self["blockref"]

							blockRef:SetClampedToScreen(true)
							blockRef:SetClampRectInsets(screenWidth, screenWidth, screenHeight, screenHeight)

							tinsert(button.unitSecureFrames, blockRef)
						end
...
function mod:PreClick(button)
	if button:blockFunc() then return end
	...
	if enable then
		...
		if not self.units then
			...
			for _, unit in ipairs(unitTypes) do
				self.units[unit] = createButton(unit)
			end
		...
		if not self["blockref"] then
			self["blockref"] = CreateFrame("Button", nil, UIParent, "SecureHandlerBaseTemplate")
			self["blockref"]:Size(1)
			self["blockref"]:Point("BOTTOMLEFT")
			self["blockref"]:SetClampedToScreen(true)
			self["blockref"]:SetClampRectInsets(0, 0, 0, 0)
			self["blockref"].dontBroom = true
		end
		...
		for _, info in ipairs(E.db.Extras.general[modName].buttons) do
			...
			self[info.name]:SetFrameRef("blockref", self["blockref"])
			self[info.name]:RegisterForClicks("AnyUp", "AnyDown")
			self[info.name]:WrapScript(self[info.name], "OnClick", [[
                self:SetAttribute("type1", "")

				local blockerPos = self:GetFrameRef("blockref"):GetRect()

				if (blockerPos < 0 and down) or (blockerPos >= 0 and not down) then
					return
				end
