...
function mod:ConfigureContainer(f, isBank, db, numColumns, buttonSize, buttonSpacing)
	...
	for _, section in ipairs(f.currentLayout.sections) do
		for _, button in ipairs(section.buttons) do
			if not f.currentLayout.filter[min(6,button.rarity or select(3,GetItemInfo(button.itemID or B_GetItemID(nil, button.BagID, button.SlotID) or -1)) or -99)] or not B.db.shownBags['bag'..button.BagID] then
				button.isHidden = true
				button:Hide()
			else
				button.isHidden = nil
...
function mod:UpdateSection(f, section, numColumns, buttonSize, buttonSpacing, resize)
	...
	elseif section.db.minimized then
		...
		for _, button in ipairs(buttons) do
			if layoutFilter[min(6,button.rarity or select(3,GetItemInfo(button.itemID or B_GetItemID(nil, button.BagID, button.SlotID) or -1)) or -99)] and IsNewItem(button.BagID, button.SlotID) then
